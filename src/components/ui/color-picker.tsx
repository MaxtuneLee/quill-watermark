...
type OpenChangeHandler = NonNullable<React.ComponentProps<typeof Popover>["onOpenChange"]>;
  ...
  const store = React.useMemo<Store>(() => {
    return {
      ...
      syncColor: (value: ColorValue) => {
        const nextHsv = rgbToHsv(value);
        if (
          stateRef.current.color.r === value.r &&
          stateRef.current.color.g === value.g &&
          stateRef.current.color.b === value.b &&
          stateRef.current.color.a === value.a &&
          stateRef.current.hsv.h === nextHsv.h &&
          stateRef.current.hsv.s === nextHsv.s &&
          stateRef.current.hsv.v === nextHsv.v &&
          stateRef.current.hsv.a === nextHsv.a
        ) {
          return;
        }

        stateRef.current.color = value;
        stateRef.current.hsv = nextHsv;
        store.notify();
      },
      setOpen: (value: boolean) => {
        ...
        if (propsRef.current.onOpenChange) {
          (propsRef.current.onOpenChange as OpenChangeHandler)(value, {} as never);
        }
...
function ColorPickerImpl(props: ColorPickerImplProps) {
  ...
  useIsomorphicLayoutEffect(() => {
    if (valueProp !== undefined) {
      ...
      const color = parseColorString(valueProp) ?? hexToRgb(valueProp, currentState.color.a);
...
function ColorPickerTrigger(props: React.ComponentProps<typeof Button>) {

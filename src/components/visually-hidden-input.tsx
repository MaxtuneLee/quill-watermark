...
function VisuallyHiddenInput<T = InputValue>(props: VisuallyHiddenInputProps<T>) {
  ...
  React.useEffect(() => {
    ...
    let serializedCurrentValue: string | boolean | undefined;
    if (isCheckInput) {
      serializedCurrentValue = checked;
    } else if (value === undefined) {
      serializedCurrentValue = undefined;
    } else if (typeof value === "string" || typeof value === "boolean") {
      serializedCurrentValue = value;
    } else {
    ...
    const setInputValue = descriptor?.set
      ? (target: HTMLInputElement, nextValue: string | boolean | undefined) => {
          descriptor.set?.call(target, nextValue);
        }
      : null;

    if (prevValue !== currentValue && setInputValue) {
      const event = new Event(eventType, { bubbles });
      setInputValue(input, serializedCurrentValue);
